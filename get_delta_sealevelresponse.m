...
[~,idx] = func_delta_areachange(ff.*QRiver_dist,DeltaSLR+DeltaSub,s,r,w,bed_h,fr);
...
for jj=1:length(slr_scenarios),
    ...
    dA(~out.idx) = nan;   
    dA_unc = func_delta_areachange_montecarlo(QRiver_dist,eval(slr_scenarios{jj}),DeltaSub,eval(slr_unc_scenarios{jj}),s,r,w,bed_h);
    dA_unc(~out.idx,:) = nan;
